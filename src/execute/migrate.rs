...
use crate::{
    ...
    state::{
        models::{Migration, MigrationError, MigrationErrorPolicy, MigrationStatus},
        storage::{
...
pub fn exec_begin_migration(
    ...
    Ok(Response::new().add_attributes(vec![
        ...
        attr("migration_name", params.name.to_owned()),
    ]))
}
...
pub fn exec_step_migration(
    ...
    let entries: Vec<StdResult<_>> = CONTRACT_ID_2_ADDR
        ...
        .take(params.batch_size.unwrap() as usize)
        .collect();
    ...
    Ok(Response::new().add_attributes(vec![
        ...
        attr("migration_name", migration_name.to_owned()),
    ]))
}
...
pub fn exec_retry_migration(
    ...
    if entries.is_empty() {
        ...
        migration.retry_cursor = next_cursor_id;
    }
...
pub fn exec_cancel_migration(
    ...
    MIGRATIONS.remove(deps.storage, &migration_name);
        ...
        .collect::<Vec<StdResult<_>>>()
    {
        ...
        MIGRATION_ERRORS.remove(deps.storage, (&migration_name, id));
    }
...
pub fn handle_migration_reply(
    ...
    match reply.result {
        SubMsgResult::Ok(_) => {
            ...
            if MIGRATION_ERRORS.has(deps.storage, (&migration_name, contract_id)) {
                MIGRATION_ERRORS.remove(deps.storage, (&migration_name, contract_id));
        ...
        SubMsgResult::Err(e) => {
            ...
            if let MigrationErrorPolicy::Abort = migration.params.error_policy {
